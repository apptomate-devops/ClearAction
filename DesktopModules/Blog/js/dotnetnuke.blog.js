...
{
    var moduleId = mid; var baseServicepath = $.dnnSF(moduleId).getServiceRoot("Blog");
    this.ajaxCall = function (type, controller, action, id, data, success, fail)
    {
        $.ajax({
            type: type, url: baseServicepath + controller + "/" + action + (id != null ? "/" + id : ""),
            beforeSend: $.dnnSF(moduleId).setModuleHeaders, data: data
        })
            .done(function (retdata)
            {
                if (success != undefined) { success(retdata) }
            }
        )
            .fail(function (xhr, status)
            {
                ...
                if (fail != undefined) { fail(xhr.responseText) }
            })
    };
    ...
    {
        this.ajaxCall("POST", "Comments", "Karma", null, { blogId: blogId, commentId: commentId, karma: 0 }, function (data) { if (data.Result === "exists") { } if (success != undefined) { success() } })
    };
    ...
    {
        this.ajaxCall("POST", "Modules", "Add", null,
            { paneName: paneName, position: position, title: title, template: template }, success)
    };
...
function displayMessage(d, b, a)
{
    var c = $("<div/>").addClass("dnnFormMessage " + a).text(b); $(d).prepend(c); c.fadeOut(3000, "easeInExpo", function () { c.remove() })
};