...
app.directive('angucompleteAlt', ['$q', '$parse', '$http', '$sce', '$timeout', '$templateCache', '$interpolate', function ($q, $parse, $http, $sce, $timeout, $templateCache, $interpolate) {
    ...
    $templateCache.put(TEMPLATE_URL,
        ...
        '<input id="{{id}}_value" name="{{id}}" data-val="{{dataVal}}" ng-disabled="disabled" tabindex="{{fieldTabindex}}" ng-class="{\'angucomplete-input-not-empty\': notEmpty}" ng-model="searchStr" ng-disabled="disableInput" type="{{inputType}}" placeholder="{{placeholder}}" maxlength="{{maxlength}}" ng-focus="onFocusHandler()" class="preventdefault {{inputClass}} autocomplete" ng-focus="resetHideResults()" ng-blur="hideResults($event)" autocapitalize="off" autocorrect="off" autocomplete="off" ng-change="inputChangeHandler(searchStr)"/>' +
          ...
          scope.showDropdown = false;
            ...
            var getHeight = setInterval(function(){ 
                if ((scope.results.length == 0 ? 1 : scope.results.length) < scope.displayNumber && $('#' + scope.id + "_dropdown").children().last() != null) {
                    ...
                    $('#' + scope.id + '_dropdown').css('height', ($('#' + scope.id + "_dropdown").children().last().outerHeight() * parseInt(scope.displayNumber) - 1) + 'px');
                    window.clearInterval(getHeight);
                }
            }, 50);
