...
namespace ClearAction.Modules.Alert.Components
{
    ...
    public class AlertController : DnnApiController
    {
        ...
        private HttpResponseMessage ChangeStatus(int AlertID, int Statusid)
        {
            try
            {
                ...
                if (AlertID > 0 && oCurrentuser.UserID > 0)
                {
                    ...
                    if (UpdateAlert(otemp, (int) Helper.AlertEnum.Status.DELETED) > 0)
        ...
        {
            return ChangeStatus(AlertID, (int)Helper.AlertEnum.Status.DELIVERED);
        }

        [HttpGet]
        public HttpResponseMessage MarkAsViewed(int AlertID)
        {
            return ChangeStatus(AlertID, (int)Helper.AlertEnum.Status.VIEWED);
        }
        [HttpGet]
        public HttpResponseMessage MarkAsDismissed(int AlertID)
        {
            return ChangeStatus(AlertID, (int)Helper.AlertEnum.Status.DISMISSED);
        }
        [HttpGet]
        public HttpResponseMessage MarkAsDeleted(int AlertID)
        {
            return ChangeStatus(AlertID, (int)Helper.AlertEnum.Status.DELETED);
        }
