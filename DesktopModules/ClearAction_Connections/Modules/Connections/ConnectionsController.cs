...
using ClearAction.Modules.Connections.Entity;
    ...
    public class ConnectionsController : DnnApiController
    {
        ...
        public HttpResponseMessage GetSuggestions(int groupId, string displayName)
        {
            try
            {
                ...
                var filterlist = FilterExcludedUsers(oUser);
                var names = (from UserInfo user in filterlist
        ...
        public HttpResponseMessage AddFaviourate(FriendDTO postData)
        {
            try
            {
                ...
                if (friend != null)
                {
                    ...
                    if (oCount != null && oCount.Count >= Helper.MaxFav)
        ...
        [HttpGet] // Added by Fali on 28th March 2018
        // [ValidateAntiForgeryToken]
        public HttpResponseMessage SendMessageFromDigitalEvent(int FromUserID, int ToUserID, string Subject, string Body, string Filename, string AttachLink)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(Filename) && !string.IsNullOrEmpty(AttachLink))
                {
                    ...
                    string stringAttachmentName = Filename.Replace("/", "");
                    stringAttachmentName = stringAttachmentName.Replace('"', ' ');
                    stringAttachmentName = stringAttachmentName.Trim();

                    string stringAttachmentURL = AttachLink.Replace("/", "");
                    stringAttachmentURL = stringAttachmentURL.Replace('"', ' ');
                    stringAttachmentURL = stringAttachmentURL.Trim();


                    if (stringAttachmentName != "" && stringAttachmentURL != "")
                    {
                        //oMessageinfo.Body += " <Attach>" + string.Format(@"<a href={0} target=""_blank"" >{1}</a>", AttachLink, Filename);
                        oMessageinfo.Body += " <Attach>" + string.Format(@"<a href=""{0}"" target=""_blank""  onclick=""window.open('{0}')"" >{1}</a>", AttachLink.Replace('"', ' ').Trim(), Filename);
                    }
        ...
        public HttpResponseMessage GetMessage(int friendId)
        {
            try
            {
                ...
                //    member.GetPrivateMessage= new DbController().GetMessage(friendId).Where(x => x.ToAccount == GetCurrentUser.UserID || x.SenderUserID == GetCurrentUser.UserID).ToList();
                if (member == null)
                    return Request.CreateResponse(HttpStatusCode.OK, new { Result = "Invalid Operation" });
        ...
        public HttpResponseMessage BlockUser(FriendDTO postData)
        {
            try
            {
                ...
                var oBlock = new Alert.Entity.UserBlock()
                {
        ...
        public HttpResponseMessage SendEmailOrMessage(int FromUserID, int ToUserID, string Subject, string Body, string Filename, string AttachLink, string sEmail)
        {
            try
            {
                ...
                new Scheduler.Components.DbController().AddEmail(oEmailSchedular);
        ...
        private IEnumerable<Member> DoSort(IEnumerable<Member> oFilteruser, string iSortBy)
        {
            switch (iSortBy)
            {
                case "0":
                case "1":
                    oFilteruser = oFilteruser.OrderByDescending(x => x.LastModified);
                    break;
                ...
                default:
                    if (isBasicSearch)
                    {
                        if (string.IsNullOrEmpty(searchTerm))
                        {
                            ...
                            //users = users.Where(x => !x.IsDeleted  && !x.IsSuperUser  && !(x.Membership.LastLoginDate == x.Membership.CreatedDate)).Select(u => u).ToList();
